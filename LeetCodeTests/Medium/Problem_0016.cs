        ...
        */
        public static void Test()
        {
            ...
            */
            ...
            var target = 1;
            Console.WriteLine(
                $"Input: {Utils.PrintArray(input)}, target: {target} -> nearest {sol.ThreeSumClosest(input, target)} (wait 2)"
            );

            input = new int[] { -1, 2, 1, -4 };
            target = 0;
            Console.WriteLine(
                $"Input: {Utils.PrintArray(input)}, target: {target} -> nearest {sol.ThreeSumClosest(input, target)} (wait -1)"
            );

            input = new int[] { -1, 2, 1, -4 };
            target = -2;
            Console.WriteLine(
                $"Input: {Utils.PrintArray(input)}, target: {target} -> nearest {sol.ThreeSumClosest(input, target)} (wait -3)"
            );
        }
