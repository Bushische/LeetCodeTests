        ...
        */
        public static void Test()
        {
            ...
            */
            ...
            var target = 0;
            Console.WriteLine(
                $"in array '{Utils.PrintArray(input)}' with target '{target}' -> '{Utils.PrintArray(sol.FourSum(input, target))}'"
            );
        }
                        ...
                        while (left < right)
                        {
                            if (sumfs + nums[left] + nums[right] == target) // emit result
                            {
                                ...
                                while ((left < right) && (nums[left] == nums[left + 1]))
                                    left++; // skip same numbers from left
                                while ((left < right) && (nums[right] == nums[right - 1]))
                                    right--; // skip same numbers from right
                                left++;
                                right--;
                            }
