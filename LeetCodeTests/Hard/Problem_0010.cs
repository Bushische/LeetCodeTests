        ...
        */
        public static void Test()
        {
            ...
            */
            ...
            Console.WriteLine(
                $"isMatch(\"aab\", \"c*a*b\") → {sol.IsMatch("aab", "c*a*b")} → true"
            );

            Console.WriteLine(
                $"isMatch(\"aabcda\", \".*\") → {sol.IsMatch("aabcda", ".*")} → true"
            );
            Console.WriteLine(
                $"isMatch(\"aabcda\", \".*e\") → {sol.IsMatch("aabcda", ".*e")} → false"
            );
...
Expected: true
        ...
        public class Solution
        {
            ...
            private IList<CharSubs> ParsePattern(string inPat)
            {
                ...
                while (chindex >= 0)
                {
                    if (inPat[chindex] == '*')
                    {
                        ...
                        resList.Insert(
                            0,
                            new CharSubs() { Char = inPat[chindex], Rule = ApplyRule.ZeroOrMore }
                        );
                    }
                    else
                    {
                        resList.Insert(
                            0,
                            new CharSubs() { Char = inPat[chindex], Rule = ApplyRule.Once }
                        );
                    }
                    chindex--;
                }
            ...
            private bool prvMatch(string str, IList<CharSubs> pat, int strIndex, int patIndex)
            {
                ...
                {
                    if ((pat[patIndex].Char == '.') || (pat[patIndex].Char == str[strIndex]))
