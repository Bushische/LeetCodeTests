        ...
        */
        public static void Test()
        {
            ...
            */
            ...
            var word2 = "abd";
            Console.WriteLine(
                $"For '{word1}' and '{word2}' -> {sol.MinDistance(word1, word2)} (waits 1)"
            );

            word1 = "abc";
            word2 = "aebwc";
            Console.WriteLine(
                $"For '{word1}' and '{word2}' -> {sol.MinDistance(word1, word2)} (waits 2 insertion)"
            );

            word1 = "aebwc";
            word2 = "abc";
            Console.WriteLine(
                $"For '{word1}' and '{word2}' -> {sol.MinDistance(word1, word2)} (waits 2 deletion)"
            );

            word1 = "qwe";
            word2 = "abc";
            Console.WriteLine(
                $"For '{word1}' and '{word2}' -> {sol.MinDistance(word1, word2)} (waits 3 replace)"
            );

            word1 = "abc";
            word2 = "cba";
            Console.WriteLine(
                $"For '{word1}' and '{word2}' -> {sol.MinDistance(word1, word2)} (waits 2 replace)"
            );
...
"acetylphenylhydrazine"
            */
            ...
            word2 = "acetyl";
            Console.WriteLine(
                $"For '{word1}' and '{word2}' -> {sol.MinDistance(word1, word2)} (waits 6 replace)"
            );
        ...
        public class Solution_too_long_and_not_optimal
        {
            ...
            private void FindVariants(string word, int changePos, int madeChanges)
            {
                if (madeChanges > FoundedMin)
                    ...
                    (changePos < word.Length)
                    && (changePos < ModelWord.Length)
                    && (word[changePos] == ModelWord[changePos])
                )
                ...
                else
                {
                    ...
                    // replace a character
                    string wrep =
                        word.Substring(0, changePos)
                        + ModelWord[changePos]
                        + word.Substring(changePos + 1);
